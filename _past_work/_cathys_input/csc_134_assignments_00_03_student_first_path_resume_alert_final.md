...
2. Create a **public** repo named `` (example: `anorris`).
3. Add and personalize `README.md` (**include your favorite **``: a category + item you choose).
...
## Mini Guide: Commit Messages (verb‑first)

Use short, present‑tense prefixes to explain *why* the change exists:

- `feat:` add a new feature (e.g., `feat: add even/odd check`)
- `fix:` correct behavior (e.g., `fix: handle invalid score`)
- `docs:` README or comments (e.g., `docs: add run instructions`)
- `style:` formatting/non-functional output tweaks
- `refactor:` restructure code without changing behavior
- `test:` add/adjust test inputs or tables

### Week 9 Teaser — “Speak C++” to your agent

Starting in **Week 9**, AI code generation becomes **allowed with accountability**: write specs/pseudocode first, prepare a test table, review `git diff`, and disclose tool use. You’ll practice asking for C++‑aware changes (e.g., const‑correctness, range‑based `for`, RAII) and integrating patches responsibly.

---

## FAQ

**What’s **``** again?** A placeholder for a *favorite thing you pick* (category + item). Example: category = “anime character”; item = “Tanjiro.”\
